 ...
 * initializing the root dentry of a newly mounted filesystem. If `mount` is NULL, the resulting
 * dentry's filesystem will be left as NULL.
 *
 * If `mount` is provided, the `fs` parameter is initialized to `mount->fs`; however, it can be
 * overrided for special files.
 */
