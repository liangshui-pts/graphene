...
/*
 * Note that checkpointing the `shim_fs` results in copying them instead of using the same global
 * objects on the remote side. Unfortunately, it's hard to do better with the current state of the
 * checkpoint system.
 *
 * That does not waste too much memory, because each global object is copied only once. However, it
 * means that `shim_fs` objects cannot be compared by pointer.
 */
BEGIN_CP_FUNC(fs) {
    __UNUSED(size);
